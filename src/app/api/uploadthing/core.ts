...
export const ourFileRouter = {
  ...
  thumbnailUploader: f({
    ...
    .middleware(async ({ input }) => {
      ...
      if (!input.videoId) throw new UploadThingError("Unauthorized")
      // find the existing video with key
      const [existingVideoWithKey] = await db
      ...
      if (!existingVideoWithKey) throw new UploadThingError("Not found")
      if (existingVideoWithKey.thumbnailKey) {
        const utapi = new UTApi()
        await utapi.deleteFiles(existingVideoWithKey.thumbnailKey)
  ...
  bannerUploader: f({
    ...
    .middleware(async () => {
      // This code runs on your server before upload
      const { userId: clerkUserId } = await auth();
      // If you don't sign in to have clerk user ID, the user will not be able to upload
      if (!clerkUserId) throw new UploadThingError("Unauthorized");
      // find the user in the database
      ...
      // check if the user have banner key
      // define a new Uploadthing api to delete the banner if it exist
